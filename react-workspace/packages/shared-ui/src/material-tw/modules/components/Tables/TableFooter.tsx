      ...
      <Suspense fallback={<div>Loading...</div>}>
          <CardFooter className="flex items-center justify-between border-t border-blue-gray-50 p-4">
              <Typography variant="small" color="blue-gray" className="flex flex-row font-normal">
                  <span className="flex flex-col justify-center mr-2" style={{ whiteSpace: 'nowrap' }}>
                      Page {currentPage} of {totalPages}
                  </span>
                  <Select label="Items per page" value={`${pageSize}`} onChange={handlePageSizeChange} color="blue">
                      {pageSizes.map((size) => (
