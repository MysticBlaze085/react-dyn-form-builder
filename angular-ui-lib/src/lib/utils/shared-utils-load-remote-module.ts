...
export async function loadRemoteModule(remoteName: string, moduleName: string) {
  ...
  const container = remoteContainerMap.has(remoteName) ? remoteContainerMap.get(remoteName) : await loadRemoteContainer(remoteName);
...
async function loadRemoteContainer(remoteName: string) {
  ...
  if (!remoteUrl) {
    throw new Error(`Could not resolve remote URL for ${remoteName}`);
  }
